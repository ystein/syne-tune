...
GPU and is much cheaper to run than ``ml.g4dn.12xlarge`` we used with the
local backend above.
...
`this tutorial <../visualization/README.html>`__. Here is the plot:
...
* In general, we obtain better results faster with more workers. However,
  especially for ``BO`` and ``MOBSTER``, the improvements are less pronounced
  than one might expect.
* Our results counter a common "wisdom", that as we go to higher degrees of
