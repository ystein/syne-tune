    ...
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        if self.test_dataloader is not None:
            output = self.trainer.evaluation_loop(
                ...
                'Testing',
                prediction_loss_only=True if self.trainer.compute_metrics is None else None,
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments, SyneTuneArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, training_args, syne_tune_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
    ...
    if st_checkpoint_dir is not None:
    ...
    split = train_dataset.train_test_split(
        ...
        seed=training_args.seed)  # fix seed, all trials have the same data split
