...
class TuRBOFIFOSearcher(GPFIFOSearcher):
    ...
    def _process_running_trials(
        ...
        new_running_trials = []
        labeled_trial_ids = set(ev.trial_id for ev in state.trials_evaluations)
        for trial_id, incumbent_trial_id in self._running_trials:
            if trial_id in labeled_trial_ids and \
                    incumbent_trial_id in labeled_trial_ids:
    ...
    def _process_running_trial(
        ...
        incumbent_prediction = predictions[1]
        is_success = (predictions[0] < incumbent_prediction -
                      1e-3 * np.fabs(incumbent_prediction))
    ...
    def _get_config_modelbased_prepare_bo(
        ...
        result = super()._get_config_modelbased_prepare_bo(
            model=model, exclusion_candidates=exclusion_candidates,
            hp_ranges=hp_ranges)
