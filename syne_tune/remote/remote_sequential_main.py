...
"""
Entrypoint script for `RemoteSequentialLauncher`, runs a number of
experiments sequentially, loading a tuner for each experiment.
"""
import logging
...
if __name__ == '__main__':
    ...
    tuners_path = Path(args.tuner_path)
    num_tuners = 0
    while (tuners_path / str(num_tuners)).exists():
        tuner_path = str(tuners_path / str(num_tuners))
        logging.info(f"load tuner {num_tuners} from path {tuner_path}")
        tuner = Tuner.load(tuner_path=tuner_path)
        ...
        tuner.tuner_path = Path('/opt/ml/checkpoints') / str(num_tuners)
        ...
        logging.info(f"starting remote tuning {num_tuners}")
        tuner.run()

        num_tuners += 1

    logging.info(f"Finished {num_tuners} tuning experiments")
